...
$sql= "SELECT * FROM product";
 ...
 $product=array();
  ...
  //Thêm session['cart']
    $sql = "SELECT product.Name, product.Size,product.Color,cart.amount,product.id_product,product.img,product.Cost from cart inner join product ON product.id_product = cart.id_sp where cart.id_us = '$id'";
    $result = $data->query($sql);
    while($row = $result->fetch_assoc())
    {
      $cartProducts[] = $row;
